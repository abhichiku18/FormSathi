...
const previewPlacementPages = document.getElementById("previewPlacementPages");
const previewStatus = document.getElementById("previewStatus");
const previewError = document.getElementById("previewError");
const downloadButton = document.getElementById("downloadButton");
const resetPlacementButton = document.getElementById("resetPlacementButton");
const restartButton = document.getElementById("restartButton");
const generatedPreviewCard = document.getElementById("generatedPreviewCard");
const downloadLink = document.getElementById("downloadLink");
const pdfViewer = document.getElementById("pdfViewer");
const previewFloatingRenderCard = document.getElementById("previewFloatingRenderCard");
const previewFloatingRenderFrame = document.getElementById("previewFloatingRenderFrame");
const previewFloatingRenderStatus = document.getElementById("previewFloatingRenderStatus");
...
const language = localStorage.getItem("formsathiLanguage") || "English";
...
function setError(message = "") {
  ...
  previewError.classList.toggle("hidden", !message);
}
...
function setRenderedPreviewState(message, pdfUrl = "") {
  previewFloatingRenderStatus.textContent = message || "Generate once to see the real PDF output here.";
  previewFloatingRenderCard.classList.toggle("hidden", !message && !pdfUrl);
  previewFloatingRenderFrame.src = pdfUrl || "";
  if (pdfUrl) {
    state.renderedPreviewUrl = pdfUrl;
  }
}

function estimateChipWidth(text) {
  const content = String(text || "");
  return Math.min(Math.max(content.length * 8.5 + 26, 54), 280);
}
...
function createTextNode(fieldIndex, field, overlay, pageSize) {
  ...
  node.style.width = `${Math.min(estimateChipWidth(value), widthRatio * overlay.clientWidth)}px`;
...
function updateDraggedField(fieldIndex, pageSize, point, offset) {
...
function updateDraggedSignature(pageSize, point, offset) {
...
function bindPageInteractions(layer, pageIndex) {
  ...
  layer.addEventListener("pointermove", (event) => {
    ...
    if (state.draggedItem.type === "field" && state.draggedItem.fieldIndex !== null) {
      updateDraggedField(state.draggedItem.fieldIndex, pageSize, point, state.dragOffset);
    } else if (state.draggedItem.type === "signature") {
      updateDraggedSignature(pageSize, point, state.dragOffset);
    }
    renderAnnotations();
  });
...
function buildPdfPage(pageIndex, pageSize) {
  ...
  const width = Math.min(760, Math.max(320, previewPlacementPages.clientWidth - 40 || 760));
  ...
  previewPlacementPages.appendChild(shell);
...
function renderSourcePages() {
  state.pageViews = [];
  previewPlacementPages.innerHTML = "";
...
async function generatePdf() {
  return renderGeneratedPdf(true);
}

async function renderGeneratedPdf(markReady = false) {
  try {
    ...
    setError("");
    previewStatus.textContent = markReady ? "Generating your filled PDF..." : "Refreshing rendered preview...";
    ...
    const response = await fetch(`${API_BASE_URL}/generate-pdf`, {
      ...
      body: JSON.stringify({
        ...
        original_filename: uploadResponse.filename,
        language,
        fields,
        form_data: formData,
        signature,
        extracted_text: uploadResponse.extracted_text,
        signature_anchor: state.signatureAnchor || null,
      }),
    });

    if (!response.ok) {
      const errorPayload = await response.json().catch(() => ({}));
      throw new Error(errorPayload.detail || "Unable to generate PDF.");
    }

    const payload = await response.json();
    const pdfUrl = `${API_BASE_URL}${payload.pdf_url}`;
    localStorage.setItem("formsathiGeneratedPdf", pdfUrl);
    pdfViewer.src = pdfUrl;
    downloadLink.href = pdfUrl;
    downloadLink.classList.remove("disabled-link");
    generatedPreviewCard.classList.remove("hidden");
    setRenderedPreviewState(
      markReady
        ? "Live render matches the generated PDF below."
        : "Live render refreshed while you adjust placement.",
      pdfUrl,
    );
    previewStatus.textContent = markReady
      ? "Your completed PDF is ready below. Open it only if you want to download or review the final file."
      : "Rendered preview refreshed. Keep dragging until it looks right.";
  } catch (error) {
    previewStatus.textContent = markReady ? "PDF generation failed." : "Rendered preview failed.";
    setError(error.message || "Unable to generate PDF.");
  } finally {
...
function resetPlacement() {
  ...
  renderAnnotations();
  previewStatus.textContent = "Placement reset to the detected positions.";
  scheduleRenderedPreview();
}
...
async function initializePreview() {
  try {
    setError("");
    previewStatus.textContent = "Loading your uploaded form preview...";
    state.metadata = await loadMetadata();
    ensureSignatureAnchor();
    renderSourcePages();
    previewStatus.textContent = "Drag any value if it needs a better position, then generate the final PDF.";
    scheduleRenderedPreview();
  } catch (error) {
    previewStatus.textContent = "Could not load the preview.";
    setError(error.message || "Unable to open the preview step.");
  }
}
